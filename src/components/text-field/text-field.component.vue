<template>
  <div :class="['text-field', ...textFieldClassModifiers]">
    ...
    <div
      v-if="captionContent || showCounter"
      class="text-field__caption caption"
    >
...
const errorMessage = props.rules
  ? useValidationProvider(validationConfig)?.errorMessage
  : "";
...
const counter = computed(
  () => model.value.length + (!!props.maxLength && ` \\ ${props.maxLength}`)
);
