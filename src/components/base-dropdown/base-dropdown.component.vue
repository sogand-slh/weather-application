...
defineProps({
  items: {
    ...
    default: () => [],
    validator: (list) =>
      list.every((item) => "label" in item && "value" in item),
  },
  placeholder: {
    type: String,
    default: "",
...
const dropdownClassModifier = useClassModifier(
  "dropdown",
  dropdownClassConditionMapper
);
...
.#{$root} {
  ...
  &__collapsible {
    ...
    right: 0;
    top: 100%;
    border-bottom-left-radius: shaper(5);
    border-bottom-right-radius: shaper(5);
    z-index: $drawer;
